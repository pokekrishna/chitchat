...
func NewMockWithDB() (*sql.DB, sqlmock.Sqlmock) {
...
func TestGETThreads (t *testing.T){
	...
	t.Run("GET on '/threads' with header 'Accept': 'foo' should appear in request context", func(t *testing.T) {
		db, mock := NewMockWithDB()
		timeT1 := time.Now()
		timeT2 := time.Now()
		threads := []data.Thread{
			data.Thread{
		...
		_ = fmt.Sprintf("[{\"Id\":1,\"Uuid\":\"uuid-sample-1\",\"Topic\":\"topic1\",\"UserId\":2,\"CreatedAt\":%s},{\"Id\":2,\"Uuid\":\"uuid-sample-2\",\"Topic\":\"topic2\",\"UserId\":5,\"CreatedAt\":%s}]",
